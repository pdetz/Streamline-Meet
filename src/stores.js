...
export const emailListStore = writable([]);
export function toggleSwimmerInEmailList(swimmer) {
    emailListStore.update(list => {
        const index = list.findIndex(s => s.id === swimmer.id);
        if (index === -1) {
            return [...list, swimmer];
        } else {
            return list.filter(s => s.id !== swimmer.id);
        }
    });
}
